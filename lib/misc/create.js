...
module.exports.createCanvasWeatherForecast = async function(locationData, weather, scale=4) {
    let startTime; // just for time-testing
    ...
    console.log(`Draw: main icon - ${Date.now()-startTime}ms`);
    // current temp
    ...
    console.log(`Draw: graph (main) - ${Date.now()-startTime}ms`);
    ...
    console.log(`Draw: graph (accessories) - ${Date.now()-startTime}ms`);
    // GRAPH END
